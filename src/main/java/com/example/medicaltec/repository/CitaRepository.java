...
public interface CitaRepository extends JpaRepository<Cita, Integer> {
   ...
   @Query(nativeQuery = true, value = "SELECT C.idcita as `ID`, S.nombre as `Sede`, E.nombre as `Title`, formapago as `FormaPago`, modalidad as `Modalidad`, T.tipo_cita as `TipoCita`,fecha as `Start`, hora as `Hora`, concat(D.nombre,\" \",D.apellido) as `Doctor` FROM cita C\n" +
           "inner join sedes S on C.sedes_idsedes=S.idsedes\n" +
